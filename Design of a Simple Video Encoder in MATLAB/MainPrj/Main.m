...
% fid_rec = fopen('reconstructed.yuv', 'wb'); % for debuging
...
for f = 1:numFrames
    ...
    if f == 1
        ...
        MVR = max(-16, min(15, MVR)); % Boundary safety
        MVC = max(-16, min(15, MVC)); % Boundary safety
        [rows, cols] = size(MVR);
        for r = 1:rows
            for c = 1:cols
                mv_bits = HuffCodingMV(MVR(r,c), MVC(r,c));
    ...
    % WriteFrame(refFrame, fid_rec); % for debuging
